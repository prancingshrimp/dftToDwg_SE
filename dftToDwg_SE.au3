...
$gui = GUICreate("dftToDwg",$guiwidth,$guiheight,2,2)
$label = GUICtrlCreateLabel("Please select folder with dft files.",5,1*$buttonheight+15,400)
GUICtrlCreateLabel("v0.11",5,2*$buttonheight+5,400)
...
;~ get all dft-files
	...
	If $proofDft = 1 Then
		...
		For $i = 1 To $fileArrayDft[0]
			...
			$fileArrayDft_[$i][2] = $path & "\" & StringReplace($fileArrayDft_[$i][0], ".dft", ".dwg")
		Next
	EndIf
	...
	If $proofLnk = 1 Then
		...
		For $i = 1 To $fileArrayLnk[0]
			...
			$fileArrayLnk_[$i][2] = $path & "\" & StringReplace($fileArrayLnk_[$i][0],".lnk",".dwg")
		Next
	EndIf
